...
struct CSVImportView: View {
                ...
                VStack(spacing: 24) {
                    ...
                    VStack(spacing: 16) {
                        ...
                        VStack(spacing: 8) {
                            ...
                            Text("Select a CSV file to import bucket list items")
                                .font(.body)
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            HStack {
                                Image(systemName: "circle.fill")
                                    .font(.system(size: 4))
                                    .foregroundStyle(.secondary)
                                Text("activity (string) - Required")
                            }
                            HStack {
                                Image(systemName: "circle.fill")
                                    .font(.system(size: 4))
                                    .foregroundStyle(.secondary)
                                Text("completed (true/false) - Required")
                            }
                            HStack {
    ...
    private func parseCSV(_ csvContent: String) throws -> [BucketListItem] {
        ...
        for lineIndex in 1..<nonEmptyLines.count {
            ...
            do {
                let item = try createBucketListItem(from: columns, using: columnMapping)
                items.append(item)
            } catch {
    ...
    private func validateHeader(_ header: [String]) throws {
        let requiredColumns = ["activity", "completed"]
        let optionalColumns = ["location", "imagePaths"]
    ...
    private func createBucketListItem(from columns: [String], using mapping: [String: Int]) throws -> BucketListItem {
        guard let activityIndex = mapping["activity"],
              let completedIndex = mapping["completed"] else {
            throw CSVImportError.missingRequiredColumn("activity or completed")
        }
        
        let activity = columns[activityIndex]
        guard !activity.isEmpty else {
            throw CSVImportError.emptyRequiredField("activity")
        }
        
        let completedString = columns[completedIndex].lowercased()
        let completed: Bool
        ...
        case "false", "no", "0":
            completed = false
        default:
            throw CSVImportError.invalidBooleanValue(completedString)
        }
        
        let location: String? = {
            if let locationIndex = mapping["location"] {
                let loc = columns[locationIndex]
...
enum CSVImportError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .unknownColumn(let column):
            return "Unknown column: '\(column)'. Only 'activity', 'completed', 'location', and 'imagePaths' are allowed."
        ...
        case .invalidBooleanValue(let value):
            return "Invalid boolean value: '\(value)'. Use 'true', 'false', 'yes', 'no', '1', or '0'."
        }
    }
}
